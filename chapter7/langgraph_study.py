...
class EducationState(TypedDict):
    """定义在整个图的工作流中传递的状态。"""
    student_profile: str   # 学生档案（初始输入，包含姓名、年级、学习表现等）
    analysis_report: str   # 学情分析报告（分析师节点的输出，记录知识薄弱点）
    resource_list: str     # 教学资源列表（研究员节点的输出，推荐的学习资源）
    learning_plan: str     # 一周学习计划（设计师节点的输出，按天安排的学习任务）
    final_letter: str      # 给家长的信件（沟通专员节点的输出，最终交付物）
