...
def producer_node(state: GraphState):
    ...
    # 生成报告
    report = producer_chain.invoke(input={
        "task": state["task"],
...
def should_continue(state: GraphState):
    """ 决定是否继续迭代 """
    # 如果报告完美或达到最大迭代次数，则结束
    if state["is_perfect"] or state["iteration"] >= state["max_iteration"]:
        return "end"
    # 否则继续生产者节点
    return "producer"
...
def create_reflection_graph():
    ...
    # 设置入口点
    workflow.set_entry_point(key="producer")
...
# 主函数
def generate_date_analysis_report_with_langgraph():
...
if __name__ == "__main__":
    generate_date_analysis_report_with_langgraph()
