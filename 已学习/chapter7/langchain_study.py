...
def main():
    ...
    overall_chain = (
        analyst_prompt | llm | output_parsers |
        {"analysis": lambda x:x} | researcher_prompt | llm | output_parsers |
        {"resources": lambda x:x} | designer_prompt | llm | output_parsers |
        {"plan": lambda x:x} | communicator_prompt | llm | output_parsers
    )
    ...
    try:
        ...
        final_letter = overall_chain.invoke(input={"student_profile": student_profile})
        print("\n------------------\n")
        print("## 给家长的最终信件 ##")
        print(final_letter)
