...
class UserIntent(TypedDict):
    ...
    destination: Optional[str]     # 目的地（创建计划时使用）
    duration: Optional[str]        # 旅行时长（创建计划时使用）
    budget: Optional[str]          # 预算（创建计划时使用）
    interests: Optional[str]       # 兴趣偏好（创建计划时使用）
    travel_date: Optional[str]     # 出行时间（创建计划时使用）
    adjustment_request: Optional[str]  # 调整请求内容（调整计划时使用）
...
class UserIntentParser(JsonOutputParser):
    def parse(self, text: str) -> UserIntent:
        try:
            # 调用父类的 parse 方法，自动提取文本中的 JSON
            result = super().parse(text)
            # 确保 intent 字段存在，不存在则设为 "unknown"
            if "intent" not in result:
                result["intent"] = "unknown"
            return result
        except Exception as e:
            # 解析失败的兜底：返回 unknown 意图
            print(f"用户意图解析错误: {e}")
...
def extract_travel_info(state: AgentState):
    ...
    # 如果最后一条消息不是用户消息（比如是 AI 消息），则不更新状态
    return state
...
def adjust_travel_plan(state: AgentState):
    ...
    result = response.invoke({
        # 将原始计划字典转为格式化的 JSON 字符串，方便大模型阅读
        "original_plan": json.dumps(state["current_plan"], indent=2, ensure_ascii=False),
        "adjustment_request": state.get("adjustment_request", "")  # 调整需求
    })
...
def route_request(state: AgentState):
    ...
    if intent == "create":
        return "create_plan"       # → 走"创建计划"节点
    elif intent == "adjust":
        return "adjust_plan"       # → 走"调整计划"节点
    else:
        return "handle_unknown"    # → 走"未知意图处理"节点
...
def build_travel_planner_graph():
    ...
    workflow.add_node("extract_info", extract_travel_info)    # 提取用户意图和信息
    workflow.add_node("create_plan", create_travel_plan)      # 创建旅行计划
    workflow.add_node("adjust_plan", adjust_travel_plan)      # 调整旅行计划
    workflow.add_node("handle_unknown", handle_unknown_intent) # 处理未知意图

    # ---- 设置入口点 ----
    # 所有请求都从 extract_info 节点开始处理
    workflow.set_entry_point("extract_info")

    # ---- 添加条件边 ----
    # 从 extract_info 出发，根据 route_request 的返回值决定走哪个节点
    # 例如：route_request 返回 "create_plan" → 流转到 create_plan 节点
    workflow.add_conditional_edges(
        "extract_info",       # 源节点
        route_request,        # 路由函数，返回目标节点名称
        {
            "create_plan": "create_plan",       # 返回 "create_plan" → 走 create_plan 节点
            "adjust_plan": "adjust_plan",       # 返回 "adjust_plan" → 走 adjust_plan 节点
            "handle_unknown": "handle_unknown"  # 返回 "handle_unknown" → 走 handle_unknown 节点
        }
    )
    ...
    # ---- 添加内存检查点 ----
    # MemorySaver 将图的状态保存在内存中，使得同一 thread_id 的多次调用可以共享状态
    # 这就是为什么 adjust_plan 能读到之前 create_plan 生成的 current_plan
    memory = MemorySaver()

    # 编译图：将定义好的节点和边编译为可执行的对象
...
class TravelPlannerAgent:
    ...
    def create_plan(self, destination, duration, budget, interests, travel_date):
        ...
        result = self.graph.invoke(
            ...
            config=self.config  # 传入 thread_id，确保状态跨调用持久化
        )
